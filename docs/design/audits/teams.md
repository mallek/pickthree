...
Dark and light at 390px, one pair per state. All 18 were regenerated by the "Your meta" copy
change's `npm run web:audit` run (2026-09-25) and converted to WebP (600px wide, quality 72).
Full-page captures show the tab bar at the true bottom of the page (see M2 below). After
sign-off, the five pairs that show team rows (`02-teams`, `teams-second-open`,
`teams-community`, `19-teams-ultra`, `teams-cup`) were re-converted the same way from the
2026-09-26 run for the number on the rows (the "After sign-off" row under Findings); the other
four converted byte for byte identical and were left as they were. The same five were
re-converted again from the Team Analysis final fix wave's `web:audit` run (2026-09-26), for the
open row's roles on one line (the second "After sign-off" row); the other four again converted
...
The Source option that weights by the player's own battle log is labeled "Your meta" ("Your
meta: 11 of 15" in the Great League captures, "Your meta: 0 of 15" in Ultra and the cup, plain
"Your meta" from 15 battles), and Your Meta's hint names it the same way. Travis, 2026-09-25:
"Instead of your log. I think we say your meta". Where "your log" means the battle log itself
("Your log never leaves this phone", the engine's assumption lines), it stays.
...
| `teams-filters-sheet`: the Filters sheet open over the list, Window at its top, disabled because the source is Your meta: its value and chevron muted, its label as it was, the Source note under it | ![](img/teams-filters-sheet-dark.webp) | ![](img/teams-filters-sheet-light.webp) |
...
- [x] `npm run lint`, `npm run typecheck`, `npm test`, `npm run check-colors`, `npm run
      ...
      workspaces; `npm test` 127 files, 1,081 tests passed; `check-colors` exit 0 (no literal
...
- [x] at most one line of text before the first result: no supporting line at all between the
      ...
      select: "Applies when Source is GBL, Tournaments or All" for PvPoke or Your meta (as in the
...
- [x] the new behaviors:
  ...
  - Filters count: `filterCount()` (`Teams.tsx`) counts the four moved switches, a non-empty
    ...
    or Your meta, so it does not count then). `teamsHeader.test.tsx`'s "counts the moved filters,
...
| Copy (Travis, 2026-09-25: "Instead of your log. I think we say your meta"): the log Source option was labeled "Your log". | The option reads "Your meta: N of 15", then "Your meta"; Your Meta's hint says to pick "Your meta" as the Source; comments and test names follow; the spec lists the Source options (`8994de4`). Tests: the label below and at 15 battles (`teamsHeader.test.tsx`), the hint (`yourMetaScreen.test.tsx`). | `23b63b2`, `8994de4` |
| After sign-off, from the Team Analysis revision (spec "Shared team components", `TeamRowSummary`, revised 2026-09-26): a collapsed row gave fit, difficulty and Stardust but not the battle number, the one figure that tells three Strong-fit teams apart ("it gives you an idea between 3 A teams who has an edge", Travis on the number, 2026-09-26). Seen in the new captures: the longer line broke "251,420 Stardust" between the number and its unit on the Zweilous row. | `TeamRowSummary` leads its line with the rounded battle number: "88 · Strong fit · Demanding · 805,320 Stardust", joined with `SEP` so no "·" starts a line, and the Stardust amount joined to its unit with `format.ts`'s `amount()` (the non-breaking join every cost line already used). `teamComponents.test.tsx` pins the whole line, non-breaking spaces included. `web:audit` re-run 2026-09-26: exit 0, zero findings on all nine enforced Teams screens in both themes, the four guards above passing. Re-converted: `02-teams`, `teams-second-open`, `teams-community`, `19-teams-ultra`, `teams-cup` (dark and light); checked by eye in both themes, every row's line wraps whole words and whole amounts. | `94fc75b`, and the commit adding this row |
| After sign-off, from the Team Analysis revision's final review (I2, a controller ruling): the open row's strip (`TeamCardBody`'s `.slots3`, a flex column per member from `packages/ui/base.css`) dropped roles onto different lines when some names wrapped: Melmetal's "SAFE SWITCH" a line above "LEAD" and "CLOSER" in `02-teams` and `teams-second-open` (whose second open row also had "CLOSER" a line below "LEAD" and "SAFE SWITCH", under Galarian Stunfisk's two-line name), and three heights in `19-teams-ultra`, where "Shadow Giratina (Altered)" wraps to three lines. The signed "rows align" tick above covered the collapsed rows, not the open row's roles. | The subgrid Team Analysis's strip uses (`.analysis-strip-member`, `analysis.md`'s Findings), scoped to the web app as `.team-body .slots3` / `.team-body .slots3 > .slot` in `app.css`: each member spans the strip's three rows (token, name, role), so every role sits on one line under the longest name. `packages/ui`'s `.slots3`, meta.pick3.gg and `WhyThisTeam`'s own `.slot` tiles (not inside `.slots3`) do not move. `web:audit` re-run 2026-09-26: exit 0, zero findings on all nine enforced Teams screens in both themes. Re-converted: `02-teams`, `teams-second-open`, `teams-community`, `19-teams-ultra`, `teams-cup` (dark and light); checked by eye in both themes, "LEAD", "SAFE SWITCH" and "CLOSER" on one line in every open row, the Ultra row's three-line Giratina name included. | `6571697`, and the commit adding this row |
| After sign-off, from the Team Analysis revision's final review (a controller ruling on which three threats the engine keeps): the open row prints `explanation.why`, whose "Biggest risk: X" names the first key threat. The engine used to keep the three most common non-wins, so a close matchup could be named over one nobody on the team beats. | The engine keeps unanswered threats first (best rating under 450), then close ones, meta rank within each (`f05e48c`, `analysis.md` Ruling 6), so "Biggest risk" now names the first unanswered threat. The captured Teams rows' lines did not change. | `f05e48c`, and the commit adding this row |

Visible changes outside Teams, from this branch:
- Your Meta's hint, shown when another Source is picked, now says to pick "Your meta" as the
  Source on Teams.
