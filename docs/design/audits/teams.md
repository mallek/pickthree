...
Dark and light at 390px, one pair per state. All 18 were regenerated by the final fix wave's
`npm run web:audit` run (2026-09-25) and converted to WebP (600px wide, quality 72).
Full-page captures now show the tab bar at the true bottom of the page (see M2 below).
...
| `teams-community`: Source set to GBL (community ladder) (full page) | ![](img/teams-community-dark.webp) | ![](img/teams-community-light.webp) |
...
- [x] at most four text levels, one page title: one `<h2>` "Your Teams" in every capture,
      ...
      totals, the weighting line) - four.
...
- [x] rows align; gutters and the 8px base hold: every collapsed row is one card with the
      ...
      pairs, and by the `screens.mjs` border guard); the header, league row, selects, weighting
      line, the Build your own team card and the list share one 20px gutter in both themes.
...
- [x] at most one line of text before the first result: one supporting line between the
      controls and the list on every list state - "PvPoke weighting until your log reaches 15
      battles" (`02-teams`, `teams-second-open`, `19-teams-ultra`, `teams-cup`,
      `teams-loading`, `teams-empty`) or "This meta · GBL weighting" (`teams-community`). In
      `teams-no-collection` the one line is "No collection yet. Pick a way in." In
      `teams-filters-sheet` the sheet opens on its title ("Filters") and the "Team style" field
      label, with no text before its first control.
...
- [x] the new behaviors:
  ...
  - Filters count: `filterCount()` (`Teams.tsx`) counts the four moved switches, a non-empty
    exclude list and a non-"Any" team style; `teamsHeader.test.tsx`'s "counts the moved filters,
    a non-empty exclude list and a non-Any team style". `teams-empty` shows it on screen:
    "Filters 1" (the exclude list) on both the header button and the Empty action.
