...
matchup grid, "Biggest risk" under Why this team, and When to switch's close lines); `03-team-detail`, `analysis-confirm` and
...
- [x] `npm run web:audit` clean for this page's screens (listed in `AUDIT_ENFORCED`), re-run
      ...
      `945ac03`, and once more after `0c42a35`): exit 0 both times, zero findings on the same
      fifteen enforced screens in both themes, 838 on screens not yet redesigned, no "Browser
      errors" section.
...
- [x] `npm run lint`, `npm run typecheck`, `npm test`, `npm run check-colors`, `npm run
      check-tokens`, all run 2026-09-26 after the final fix wave's last code commit (`0c42a35`):
      lint exit 0; typecheck exit 0 across all seven workspaces; `npm test` 134 files, 1,193
...
- [x] tests cover the new behavior: `apps/web/test/analysisComponents.test.tsx` (the hero card:
      ...
      `packages/engine/test/explain/explain.test.ts` (key threats: the unanswered ones first
      and kept over close ones that outrank them, meta rank first within each group, "Biggest
      risk" naming the first; the switch plan: a lead loss nobody wins reads "Close; shields
      decide it. Best try: ..." in the close band, the lead included when it comes closest, and
      "Nobody on the team beats it. ..." under it, the unanswered listed first, then the close
      ones, then the ones with a switch),
...
6. **Which three threats the engine keeps (controller's ruling in the final fix wave, on the
   ...
   Jellicent). `f05e48c`; `explain.test.ts` (first named `keyThreats.test.ts`) pins the selection and the order. Because
...
| Seen in the fix wave's own captures, ruled by the controller: "unanswered" meant two things. Threats called an opponent close when the team's best rating was 450 or more, while When to switch said "Nobody on the team beats it" whenever no slot won a lead loss, so the same opponent read both ways: Hisuian Electrode in `14-custom-team`, Hisuian Electrode and Medicham in `14b-custom-unranked`, Feraligatr in `14c-shared-team` (close under Threats' rules, "Nobody on the team beats it" under When to switch). | One meaning in `explain.ts`: a lead loss nobody wins reads like the threat line when the best slot on the whole team (as the key threats pick it, the lead included) rates it 450 or more, "Close; shields decide it. Best try: <that Pokémon>."; under that it stays "Nobody on the team beats it. Shield, farm energy, and switch on your terms." One constant, `CLOSE_RATING`, holds the 450 for both. The plan now sorts the unanswered first, then the close ones nobody wins, then the ones with a switch, so the intro ("Unanswered threats first") stays true. Four switch-plan tests in `explain.test.ts`. Seen in the re-converted 14, 14b and 14c (both themes). The switch line ends with a period and the Threats line does not, as each did before. | `0c42a35` |
