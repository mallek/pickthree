...
| After sign-off, from the Teams controls change: every Select's chevron sat 8px below the select's vertical middle. `Chevron`'s inline `rotate` transform replaced `.select-wrap svg`'s `translateY(-50%)` | Centered with `top: 0; bottom: 0; margin: auto 0`, the select made a block; `ui:audit` now fails when any gallery select's chevron center is more than 1px off the select's center (it reported 8.0px before). Re-run 2026-09-25: clean in dark and light, both images recaptured | 514e550 |
| After sign-off, for the Teams controls change: `FilterButton` gains an icon-only form (`iconOnly`), IconButton's 44px shape, named "Filters" or "Filters, N on", the count as a badge over the top-right corner (--accent-lo fill, --on-accent text, a 2px --bg ring). The gallery shows it with none on and one on. The page audit's clipping check skips overflow explained only by a descendant marked `data-audit-overhang` (the badge) | New form in `packages/ui/src/components/FilterButton.tsx` and `base.css`, ui tests in `Controls.test.tsx`; `scripts/audit.mjs` | 91b4e00 |
| After sign-off, from the Teams controls review: a disabled Select used the browser's own look (greyed text, and a faded field in Safari) | `.select-wrap select:disabled` in `base.css`: value in `--muted` (with `-webkit-text-fill-color`), chevron in `--faint`, opacity kept at 1 so the label and field stay put. The gallery's Select section shows a disabled Window. `ui:audit` re-run 2026-09-25: clean in dark and light; both images recaptured | 7513306 |
