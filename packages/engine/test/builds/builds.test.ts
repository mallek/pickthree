...
describe.skipIf(!haveStaticData())('eligibility', () => {
  ...
  it('rookidee at level 1 yields the later stages only', () => {
    const s = fakeSpecimen({ speciesId: 'rookidee', ivs: { atk: 15, def: 14, sta: 13 }, level: { min: 1, max: 1 } });
  ...
  it('a Pokemon already over the cap yields nothing', () => {
    const s = fakeSpecimen({ speciesId: 'snorlax', ivs: { atk: 15, def: 15, sta: 14 }, level: { min: 27.5, max: 27.5 } });
    expect(buildsFor(s, index, DEFAULT_BUILD_OPTIONS)).toEqual([]);
  });

  it('a Great League ready medicham keeps its level', () => {
    const s = fakeSpecimen({ speciesId: 'medicham', ivs: { atk: 7, def: 15, sta: 14 }, level: { min: 49, max: 49 } });
    const builds = buildsFor(s, index, DEFAULT_BUILD_OPTIONS);
  ...
  it('honors the XL and shadow filters', () => {
    const s = fakeSpecimen({ speciesId: 'medicham', ivs: { atk: 7, def: 15, sta: 14 }, level: { min: 30, max: 30 } });
    const noXl = buildsFor(s, index, { ...DEFAULT_BUILD_OPTIONS, allowXl: false });
    expect(noXl.every((b) => b.level <= 40)).toBe(true);
    const shadow = fakeSpecimen({ speciesId: 'quagsire_shadow', shadow: true, level: { min: 20, max: 20 } });
  ...
  it('skips blank-IV specimens and ineligible species', () => {
    expect(buildsFor(fakeSpecimen({ speciesId: 'azumarill', ivs: null }), index, DEFAULT_BUILD_OPTIONS)).toEqual([]);
    expect(buildsFor(fakeSpecimen({ speciesId: 'mewtwo', level: { min: 1, max: 1 } }), index, DEFAULT_BUILD_OPTIONS)).toEqual([]);
  });
});
...
describe.skipIf(!haveStaticData())('movesets', () => {
  ...
  it('marks TM badges from scanned moves and elite lists', () => {
    const m = recommendMoveset('quagsire_shadow', rankings, { fast: 'MUD_SHOT', charged: [] }, { allowEliteTm: true }, index);
    expect(m.fast.tm).toBe('have');
    const aqua = recommendMoveset('quagsire_shadow', rankings, { fast: null, charged: ['AQUA_TAIL'] }, { allowEliteTm: true }, index);
...
describe.skipIf(!haveStaticData())('evolution candy and cost', () => {
  ...
  it('uses overrides and the heuristic', () => {
    ...
    expect(evolutionCandy('rookidee', 'corvisquire', index)).toEqual({ candy: 50, estimated: false });
  ...
  it('costs a power-up only build with a second move unlock', () => {
    const s = fakeSpecimen({ speciesId: 'azumarill', ivs: { atk: 4, def: 15, sta: 13 }, level: { min: 40, max: 40 } });
    const [build] = buildsFor(s, index, DEFAULT_BUILD_OPTIONS);
    const moveset = recommendMoveset('azumarill', rankings, s.currentMoves, { allowEliteTm: true }, index);
  ...
  it('adds evolution candy and shadow multipliers', () => {
    const s = fakeSpecimen({ speciesId: 'rookidee_shadow', shadow: true, ivs: { atk: 15, def: 14, sta: 13 }, level: { min: 1, max: 1 } });
    const corv = buildsFor(s, index, DEFAULT_BUILD_OPTIONS).find((b) => b.speciesId === 'corviknight_shadow');
    expect(corv).toBeDefined();
    const moveset = recommendMoveset('corviknight_shadow', rankings, s.currentMoves, { allowEliteTm: true }, index);
    const cost = buildCost(corv as NonNullable<typeof corv>, moveset, index);
    expect(cost.evolutionCandy).toBe(150);
    const plain = costToLevel(1, (corv as NonNullable<typeof corv>).level, { shadow: false, purified: false, lucky: false });
  ...
  it('runs over the whole fixture without throwing', () => {
    ...
    for (const s of specimens) {
      for (const b of buildsFor(s, index, DEFAULT_BUILD_OPTIONS)) {
        const m = recommendMoveset(b.speciesId, rankings, s.currentMoves, { allowEliteTm: true }, index);
