...
| **Import a file**                 | A CSV export from whatever IV checker you use, or a sheet of your own. pick3 reads the columns by what is in them, not by their names: it needs a name, CP and the three IVs and works out the rest. There is a blank template to fill in at `/pick3-template.csv`. On Android with pick3 installed you can share the file straight into it. |
